...
class TestCaseWaiting:
    ...
    def test_case_waiting(self, before, username, password, sender_phone_no, receiver_phone_number, sender_id, whatsapp_contact_no, whatsapp_alt_contact):
        driver = before

        allure.dynamic.title("登录账号参数化测试_%s" % username)
        driver.get('https://web.whatsapp.com')
        WaitingMessages(driver).test_bot_expiry_queue_wait_case_wait(contact=whatsapp_contact_no, alt_contact=whatsapp_alt_contact)
